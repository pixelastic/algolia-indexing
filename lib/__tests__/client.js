...
describe('client', () => {
  ...
  const clearObjectsWait = vi.fn();
  const setSettingsWait = vi.fn();
  const mockClient = {
    multipleBatch: vi.fn(),
    copyIndex: vi.fn(),
    initIndex: vi.fn(),
  ...
  const copyIndexWait = vi.fn();
  const moveIndexWait = vi.fn();
  const multipleBatchWait = vi.fn();
  beforeEach(() => {
    vi.spyOn(current, '__algoliasearch').mockReturnValue(mockClient);
    vi.spyOn(current, 'index').mockReturnValue(mockIndex);
    ...
    vi.spyOn(pulse, 'emit').mockReturnValue();
    current.init();
  });
  ...
  describe('clearIndexSync', () => {
    ...
    it('should emit start and end events', async () => {
      vi.spyOn(current, '__uuid').mockReturnValue('abcdef');
      await current.clearIndexSync('indexName');
  ...
  describe('copyIndexSync', () => {
    ...
    it('should create the source first if it does not exist', async () => {
      ...
      vi.spyOn(current, 'createIndexSync').mockReturnValue();
  ...
  describe('configureReplicasSync', () => {
    ...
    it('should update settings of all replicas', async () => {
      ...
      const mockIndices = {};
      vi.spyOn(current, 'index').mockImplementation((indexName) => {
        const namedIndex = {
          setSettings: vi.fn().mockReturnValue({ wait() {} }),
        };
  ...
  describe('runBatchSync', () => {
    ...
    it('should use a default batch size and concurrency', async () => {
      vi.spyOn(config, 'read').mockImplementation((key) => {
        const defaultValues = {
          batchMaxSize: 2,
