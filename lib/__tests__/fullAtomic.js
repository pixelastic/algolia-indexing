const module = require('../fullAtomic.js');
const client = require('../client.js');

describe('fullAtomic', () => {
  describe('addUniqueObjectIdsToRecords', () => {
    beforeEach(async () => {
      jest.spyOn(module, '__hashMethod').mockReturnValue();
    });

    it('should add a unique object id to all records', () => {
      const input = [{ name: 'adam' }, { name: 'bob' }];
      module.__hashMethod.mockImplementation(item => {
        return item.name;
      });

      const actual = module.addUniqueObjectIdToRecords(input);
  ...
  describe('getRemoteObjectIds', () => {
    beforeEach(async () => {
      jest.spyOn(client, 'getAllRecords').mockReturnValue();
    });
  ...
  describe('buildManifestBatch', () => {
    ...
    it('should limit the number of ids per record', () => {
      jest.spyOn(module, 'manifestIdsPerRecord').mockReturnValue(1);
      const input = [{ objectID: 'foo' }, { objectID: 'bar' }];
