const current = require('../fullAtomic.js');
const client = require('../client.js');

describe('fullAtomic', () => {
  describe('addUniqueObjectIdsToRecords', () => {
    beforeEach(async () => {
      jest.spyOn(current, '__hashMethod').mockReturnValue();
    });
  ...
  describe('buildManifestBatch', () => {
    it('should return a batch of all object ids', () => {
      ...
      const actual = current.buildManifestBatch(input, 'my_index');
    ...
    it('should limit the number of ids per record', () => {
      jest.spyOn(current, 'manifestIdsPerRecord').mockReturnValue(1);
      const input = [{ objectID: 'foo' }, { objectID: 'bar' }];
