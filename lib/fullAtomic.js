...
module.exports = {
   ...
   **/
  recordHash(record) {
    if (!this.__hashMethod) {
      this.__hashMethod = nodeObjectHash().hash;
    }
   ...
   * @param {object} settings Setting to set to the index
   **/
  async run(credentials = {}, userRecords, settings) {
    client.init(credentials.appId, credentials.apiKey);

    const indexName = 'reddimg';
    ...
    try {
      ...
      // Update settings on the index first (it's faster when index is empty)
